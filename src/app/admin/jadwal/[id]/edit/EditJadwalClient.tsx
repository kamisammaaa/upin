...
import { updateJadwalUjian } from '@/app/actions/jadwal';
import Link from 'next/link';
import { ArrowLeft, Save, Edit3, Type, BookOpen, Clock, Users } from 'lucide-react';

function formatDateTimeForInput(val: Date | string) {
  if (!val) return '';
...
export default function EditJadwalClient({ 
  jadwal,
  bankSoals, 
  kelass,
  ...
  const [selectedKelas, setSelectedKelas] = useState<number[]>(
    jadwal.kelas?.map((k: any) => k.id) || []
  );
  const [acakSoal, setAcakSoal] = useState<boolean>(jadwal.acakSoal ?? false);
  const [acakOpsi, setAcakOpsi] = useState<boolean>(jadwal.acakOpsi ?? false);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await updateJadwalUjian(jadwal.id, formData);
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-2xl font-bold text-white tracking-wide">Edit Jadwal Ujian</h2>
          </div>
          <p className="text-sm text-gray-400 mt-1">Perbarui rincian jadwal, waktu pengerjaan, dan target kelas peserta ujian.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-crypto-card p-5 sm:p-8 rounded-2xl border border-crypto-border space-y-8">
        ...
        <input type="hidden" name="tipeUjian" value={jadwal.tipeUjian || 'REGULER'} />
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select 
                name="bankSoalId"
                defaultValue={jadwal.bankSoalId}
                required
                className={selectCls}
              >
            ...
            {bankSoals.length === 0 && (
              <p className="text-sm text-yellow-400/90 mt-2 bg-yellow-500/10 border border-yellow-500/20 p-2.5 rounded-lg">
                Tidak ada bank soal yang tersedia untuk dipilih. Semua bank soal sudah digunakan pada jadwal ujian lain atau belum dibuat.
              </p>
            )}
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  name="waktuMulai"
                  type="datetime-local"
                  defaultValue={formatDateTimeForInput(jadwal.waktuMulai)}
                  required
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  name="waktuSelesai"
                  type="datetime-local"
                  defaultValue={formatDateTimeForInput(jadwal.waktuSelesai)}
                  required
                  className={`${inputCls} [color-scheme:dark]`}
                />
              </div>
          ...
          <div className="pt-4 flex flex-col sm:flex-row gap-6 border-t border-crypto-border">
            ...
            <label className="flex items-center gap-4 cursor-pointer group">
              ...
              <div>
                ...
                <span className="block text-xs text-gray-500">Mengacak urutan pilihan A, B, C, D, E</span>
              </div>
            </label>
          </div>
        </div>
