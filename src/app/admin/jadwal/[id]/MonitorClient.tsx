...
import { getAnalisisSoalData, forceSubmitSesi, resetLoginSiswa } from '@/app/actions/monitor';
import { Users, CheckCircle2, Clock, AlertTriangle, Printer, ArrowLeft, RefreshCw, Download, BarChart2, List, Wifi, WifiOff, StopCircle, PlayCircle } from 'lucide-react';
...
export default function MonitorClient({ initialData, jadwalId }: { initialData: any, jadwalId: number }) {
  ...
  useEffect(() => {
    ...
    const connectSSE = () => {
      ...
      es.onerror = () => {
        ...
        es.close();
        // Browser will automatically retry; we just update badge
  ...
  const handleForceSubmit = async (sesiId: number) => {
    if (!confirm('Yakin ingin memaksa selesai ujian siswa ini? Nilai akan dihitung berdasarkan jawaban yang sudah ada.')) return;
