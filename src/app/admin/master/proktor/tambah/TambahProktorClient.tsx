...
import { createProktor } from '@/app/actions/proktor';
import Link from 'next/link';
import { ArrowLeft, Shield } from 'lucide-react';

export default function TambahProktorClient() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await createProktor(formData);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Tambah Proktor</h1>
          <p className="text-gray-400 text-sm">Buat akun baru untuk proktor ujian</p>
        </div>
      </div>
      ...
      <div className="w-full bg-crypto-card border border-crypto-border rounded-2xl overflow-hidden shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            ...
            >
              {isLoading ? 'Menyimpan...' : 'Simpan Proktor'}
            </button>
          </div>
        </form>
      </div>
